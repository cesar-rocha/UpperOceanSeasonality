...
\titlerunninghead{Seasonality in governing dynamics at submesoscales}
...
\title{Seasonality in governing dynamics at submesoscales}
%
...
\keypoints{\item  Upper-ocean submesoscale (10-100 km) turbulence and inertia-gravity waves
           ...
           \item  Submesoscale turbulence dominates the horizontal velocity and
            ...
            \item Inertia-gravity waves dominate the the horizontal velocity and
...
forward Massachusetts Institute of Technlogy general circulation model (MITgcm)
...
the skillful comparisons of model stratification to Argo climatology is necessary
for the analysis in this paper (supplemental material).
\begin{figure*}[ht]
\begin{center}
\hspace{-1.25cm}\includegraphics[width=.6\textwidth]{figs/fig1_1.pdf}\\
\vspace{-.125cm}
\includegraphics[width=.65\textwidth]{figs/fig1_2.pdf}
 \caption{(a) The study region with the subregion where the LLC outputs are
          analyzed. Colors represent the topography and black lines are absolute
          dynamic topography contours every 0.1 m. LLC 4320 (1/48$^\circ$) snapshots of surface vorticity (b and c) and a transect
...
the local planetary vorticity, and root-mean-square (RMS) of about 0.4 f. In early
...
the latter is dominantly in geostrophic balance
are overstated \citep{sasaki_etal2014,callies_etal2016}; it is only the daily-averaged fields that are to a large extent
in  geostrophic balance (the jPDF of daily-averaged vorticity vs. laplacian of SSH
is a ellipse with large eccentricity with main axis tilted by 45$^\circ$; figure \ref{fig3}n).
...
Of course daily-averaging the model fields filter out the ageostrophic flows and
...
trends were removed, and the ensuing fields were multiplied by a two-dimensional Hanning spectral ``window''.
The two-dimensional spectra were averaged azimuthly \citep[e.g., ][]{rocha_etal2016} .We discuss only spectra for the 1/48$^\circ$, which simply extends the 1/24$^\circ$
...
alternative explanation is that the shape of the baroclinic modes significantly
change seasonally. In particular the surface amplitude of the baroclinic modes are
largest when the upper ocean is strongly stratified in late Summer/ early Fall
...
Summer is a consequence of both  of submesoscale turbulence and
...
\begin{figure*}[ht]
 ...
 \caption{Seasonal variation of joint probability distributions: vorticity vs.
          ...
          in the middle of the domain (32.5$^\circ$ N). Dashed lines in (f) through (l)
          represent pure strain flow $\alpha = \zeta$. Dashed lines in (m) through
          (o) represent geostrophic flow $\zeta = \frac{g}{f}\nabla_h^2 \eta$.}
 \label{fig3}
 \end{center}
\end{figure*}
...
\subsection*{How well does the LLC simulations captures high-frequency modes?}
