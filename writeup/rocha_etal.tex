...
using a linearized equation. The model output was provided by D. Menemenlis
(JPL/NASA).
...
from other modeling studies that investigated upper dynamics
...
spanning 155-175$^\circ$E; 25-40$^\circ$N. We focus on the surface velocity field
and near-surface stratification.

\section{Seasonality in surface velocity and near-surface stratification}

We calculate the vertical vorticity, horizontal divergence, and the magnitude
of the lateral rate of strain:
...
S \equiv [(v_x+u_y)^2 + (u_x-v_y)^2]^{1/2}\, ,
\end{equation}
The velocity gradients are calculated using a centered
second-order finite differences scheme. 
