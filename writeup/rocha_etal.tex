...
\title{Seasonality in governing submesoscale dynamics in the Kuroshio Extension}
%
...
\authors{Cesar B. Rocha et al. \altaffilmark{1}}
...
\begin{abstract}
    ...
    re-stratification weakens submesoscale turbulence, it also enhances inertia-gravity
...
\end{equation}
The components of the lateral velocity gradient tensor are calculated using a centered
...
Indeed, the seasonal cycle of vorticity and rate of strain are strong winter~\ref{fig2}):
...
The vorticity and strain rate winter \ref{fig2}a-b) show that most of the lateral divergence is associated
...
winter \ref{fig3}g). Even in April, only the daily-averaged fields
are largely
...
%are overstated \citep{sasaki_etal2014,callies_etal2016};
...
residual fields
winter \ref{fig3}j and \ref{fig3}k), which is an ellipse aligned in the vertical axis.
...
Our main finding is that in global simulations with embedded tides the
...
the upper ocean. Of course, this study focuses on a single patch of ocean in the
